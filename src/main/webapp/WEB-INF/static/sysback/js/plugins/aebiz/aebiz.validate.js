        ...
        $('.form-validate').each(function() {
            ...
            $("#" + id).validate({   //验证表单
                ...
                errorPlacement: function(error, element) {  //输入错误时的提示标签显示的位置
                    ...
                    }else if(element.parents(".y_validataappend").length > 0) {
                        element.parents(".y_validataappend").append(error);
                    }else if(element.parents(".form-group").find(".thumbnail").length > 0){
                    		element.parents(".form-group").find(".thumbnail").after(error);
                    }else{
                        element.after(error);
                    }
                },
