			...
			<div class="footer">
			  <div class="container clearfix">
			      ...
			      <dl>
			        ...
			        <c:if test="${!empty cc.value}"> 
						<c:forEach items="${cc.value}" var="content">
							<dd><a href="
								<c:if test="${content.contentType eq '1'}">${pageContext.servletContext.contextPath}/frontshow/content/help/1/${content.uuid}</c:if>
								<c:if test="${content.contentType eq '2'}">${content.url}</c:if>">${content.contentTitle}</a></dd>
						</c:forEach>
				  	</c:if>
			      </dl>
			      </c:forEach>
		    	...
		    	<dl class="phone">
		    		...
		    		<dd>
		    			<p>客服中心热线：</p>
		    			<strong><aebizs:platConfig codeName='servicePhone'/></strong>	
		    		</dd>	
		    	</dl>
		    	<dl class="email">
		    		<dt></dt>
		    		<dd>
		    			<p>客服中心邮箱：</p>
		    			<strong><aebizs:platConfig codeName='serviceEmail'/></strong>	
		    		</dd>	
		    	</dl>	
		    </div>
			  </div><!-- End .container -->
