...
<form id="mainForm" action="${pageContext.servletContext.contextPath}/sysback/sysdeptmenurel/tosaverel/${m.uuid}" method="post" class='form-horizontal form-bordered form-validate'>				
		<div class="container-fluid">
				...
				<div class="breadcrumbs">
					<ul>
						<li>
							<span><aebiz:showTitle titleId="sysdept.manTitle"/></span>
		...
		<!--右侧浮动定位-->
		<div class="navbar-right y_authright">
			<ul>
				<c:forEach items="${topList}" var="smmOne">	
		...
		<div class="y_fixedbtn">
			<input type="submit" class="btn btn-primary a_size_1 btn-large" value='<aebiz:showTitle titleId="basebusiness.showmessage.save"/>'>
			<button type="button" class="btn a_size_1 btn-large cancel"><aebiz:showTitle titleId="basebusiness.showmessage.return"/></button>
		</div>
	    ...
	    function getTargetTop(elem){
			...
			$(window).scroll(function(){
				...
				if($(window).scrollTop() > 41){
					$(".y_authright").addClass("y_authhover");
				}else{
					$(".y_authright").removeClass("y_authhover");
				}
			});	
    });
	</script>
